...
namespace Boilerplate
{
    ...
    public class ExtratoBancario
    {
        ...
        public double CalcularSaldo()
        {
            ...
            {
                saldoFinal += Movimentacoes[i].ValorTransacao;
            }
            return saldoFinal;
        }
        ...
        public List<string> ObterSaldosDiarios()
        {
            ...
            //quero as movimentações de cada dia

            Dictionary<DateTime, double> valoresPorDia = new Dictionary<DateTime, double>();
            double saldo = ValorInicial;
            for(int i = 0; i < Movimentacoes.Count; i++)
            {
                DateTime dia = Movimentacoes[i].DataInclusao.Date;
                if (!valoresPorDia.ContainsKey(dia))
                {
                    valoresPorDia[dia] = saldo;
                }
                valoresPorDia[dia] += Movimentacoes[i].ValorTransacao;     
                saldo += Movimentacoes[i].ValorTransacao;
            }
            List<string> extratoPorDia = new List<string>();
            
            foreach(DateTime dia in valoresPorDia.Keys)
            {
                string texto = $"valor da movimentação no dia {dia.Date} é {valoresPorDia[dia.Date]}";
